...
class CarRentActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView.getMapAsync(this@CarRentActivity)
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult) {
                ...
                if (previousLocation != null) {
                    if (isUserNearCarMarker(yourLocation)) {
    ...
    private fun stopRenting(car: Car) {

        stopTimer()
        stopDistanceTracking()

        updatePoints()
        CarUtils.changeCarStatus(car, false)
        showRateDialog(car)
    }
        ...
        btnRate.setOnClickListener {
            ...
            fusedLocationClient.lastLocation
                .addOnSuccessListener { location ->
                    if (location != null) {
                        CarUtils.rateAndUpdateCar(car, rating, location)
                    } else {
    ...
    private fun isUserNearCarMarker(userLocation: LatLng): Boolean {
        val carLocation = LatLng(car.latitude, car.longitude)
        val distanceThreshold = 100.0

        val distance = CarUtils.calculateDistance(userLocation, carLocation)
